...
// TODO
// В эту структуру кидаем все системные констаны (жесткости людей, коэффициент
// коллективизации и тд) Короче все что не поле класса для мошеров глобально
struct SystemConstants {
  ...
  SystemConstants(float rs, float ps, float pra, float pm, float prad,
                  float pgv) {
...
struct MoshPit {
  ...
  MoshPit() = default;
  MoshPit(std::vector<Mosher> peop, float step, float leng, float wid, float pf,
          float nop, float fluct, float flock, float rov,
          SystemConstants consts)
    ...
    total_cells = cell_per_side *
                  cell_per_side; // Строго квадрат, чтобы индексы не ломались
    net.resize(total_cells);
  }
  // Далее функция для определения, к какой ячейке сетки относится чел
  // Пока тупо для удобства будем считать длину и ширину одинаковыми, причем
  // кратными range
  int32_t find_cell(const Vector &current_position) const {
    ...
    if (x_cell < 0)
      x_cell = 0;
    if (x_cell >= cell_per_side)
      x_cell = cell_per_side - 1;

    if (y_cell < 0)
      y_cell = 0;
    if (y_cell >= cell_per_side)
      y_cell = cell_per_side - 1;
  ...
  void find_neigbour_ids(int32_t cell, std::vector<int32_t> &ids) const {
    ...
    for (int32_t dy = -1; dy < 2; dy++) {
      for (int32_t dx = -1; dx < 2; dx++) {
        ...
        if (neighb_x >= 0 && neighb_x < cell_per_side && neighb_y >= 0 &&
            neighb_y < cell_per_side) {
  ...
  void
  find_neigbs_for_flock_and_rep(const Mosher &person,
                                const std::vector<std::vector<Mosher *>> &net,
                                const std::vector<int32_t> &neighbours_id,
    ...
    for (auto id : neighbours_id) {
      for (auto &mosher : net[id]) {
        ...
        if (delta_position < (mosher->radius + person.radius) * (mosher->radius + person.radius) && delta_position > 1e-8) {
  ...
  Vector get_repulsion_force(const Mosher &person,
                             std::vector<Mosher *> &repulsion_neighbs) {
    Vector full_force = {0, 0};
    for (auto &mosher : repulsion_neighbs) {
      auto diff = mosher->position - person.position;
      auto coefficent =
          (1 - diff.get_magnitude() / (person.radius + mosher->radius));
      auto repulsion_force = constants.repulsion_strength * coefficent *
                             std::sqrt(coefficent) *
                             (diff / diff.get_magnitude());
  ...
  Vector get_propulsion_force(const Mosher &person) {
    ...
    auto velocity_diff =
        (person.goal_velocity_abs - person.current_velocity.get_magnitude());
    return constants.propulsion_strength * velocity_diff *
           (person.current_velocity / person.current_velocity.get_magnitude());
  }

  // Далее сила возникающая возникающая в результате коллективизации
  Vector get_flocking_force(const Mosher &person,
                            std::vector<Mosher *> &flock_neighbs) {
    ...
    auto flock_force =
        flocking_strength * flock_vector / flock_vector.get_magnitude();
    return flock_force;
  }
  ...
  Vector get_total_determ_force(const Mosher &person,
    ...
    find_neigbs_for_flock_and_rep(person, net, neighbours_id, repulsion_neighbs,
                                  flock_neighbs);
  ...
  Vector get_stochastic_step(const Mosher &person) {
    ...
    auto stochastic_step = constants.propulsion_strength * fluct_strength *
                           sqrt_dt / person.mass * rand_vec;
    return stochastic_step;
  }
  // Для начала пересчитываем все детерминированные силы
  void set_determ_force(std::vector<std::vector<Mosher *>> &net,
                        std::vector<int32_t> &neighbours_id,
                        std::vector<Mosher *> &flock_neighbs,
                        std::vector<Mosher *> &repulsion_neighbs) {
    for (auto &mosher : people) {
      mosher.total_strenth = get_total_determ_force(
          mosher, net, neighbours_id, flock_neighbs, repulsion_neighbs);
    }
  }

  void make_step_one_agent_active(Mosher &person,
                                  std::vector<std::vector<Mosher *>> &net,
                                  std::vector<int32_t> &neighbours_id,
                                  std::vector<Mosher *> &flock_neighbs,
                                  std::vector<Mosher *> &repulsion_neighbs) {
  ...
  void make_step_one_agent_passive(Mosher &person,
                                   ...
                                   std::vector<Mosher *> &repulsion_neighbs) {
    auto determ_force = person.total_strenth;
    person.current_velocity =
        person.current_velocity + determ_force * time_step / person.mass;
    person.position = person.position + person.current_velocity * time_step;
  }
